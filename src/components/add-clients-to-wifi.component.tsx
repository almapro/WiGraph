...
export const AddClientsToWifiComponent = () => {
  ...
  const handleMacAddressChange = (
    ...
    newClients[index] = {
      ...newClients[index],
      macAddressParts: newClients[index].macAddressParts.map((p, i) =>
        i === part ? value : p,
      ),
    };
  ...
  const handleOnSubmit: React.FormEventHandler = async (e) => {
    ...
    const macAddresses = clients.map((c) =>
      c.macAddressParts.join(":").toLowerCase(),
    );
    ...
    const hasInvalidMac = clients.some(
      (client) =>
    ...
    if (hasInvalidMac) {
      enqueueSnackbar("Please enter valid MAC addresses for all clients", {
        variant: "error",
      });
      return;
    }
    ...
    try {
      ...
      enqueueSnackbar(`${clients.length} client(s) added successfully`, {
        variant: "success",
      });
    ...
    <Modal show={showAddingClientsToWifi} onClose={handleClose}>
      <form onSubmit={handleOnSubmit}>
        <ModalHeader>
          Add Clients to{" "}
          <i>
            {(activeNode as WifiNode | null)?.data.essid} -{" "}
            {(activeNode as WifiNode | null)?.data.bssid}
          </i>
        </ModalHeader>
        <ModalBody>
